...
As shown in this paper, significant amounts of data can be gathered from the imo
Android application, mostly from a mobile device's local storage. This data
includes sent and received messages, images and video's used in chats, what
calls were made and when, and what contacts are in this user's imo contact list.
Only little significant data could be obtained from monitoring a device's
network traffic as the data sent and received by imo is not in plaintext. The
only data we could see travelling over the network data related to stickers that
are used in chats. We were unable to find any identifiable location data stored
or sent by imo, and it remains unclear what location data is collected by the
imo application, if any.
...
Currently, we were only able to identify from which phone number or imo contact
a message was received. However, since one can log into the same account (i.e.
use the same phone number) on multiple devices concurrently, one cannot discern
which device sent a message. We think investigation into identifying from which
device a message was sent could prove valuable.
...
% ====
%
% - Say something about the reasoning behind choosing to limit the
%   investigation to the Android version? (prevalence on this platform, cheap android devices, probably similar on other platforms anyway, and of course just scoping/focusing our efforts)
